library(reshape2)
library(ggplot2)
#v4.2.1 (seqence -> sequence)
AddCIGAR <-
  function(data, target, Range = 10, filter_ratio=0.1){
    ...
    for(f in 1:length(psa2)){
      ...
      #If there are multiple deletions, shift the second or later range according to the previous one
      newDel <- Del
      num_Dels <- sapply(Del, length)
      n_Dels <- which(num_Dels > 1)
      
      for(j in n_Dels){
          tmpDel <- Del[[j]]
          
          for(k in 2:num_Dels[[j]]){
              tmpDel[k] <- shift(tmpDel[k], shift = width(tmpDel)[k-1])
          }
          newDel[[j]] <- tmpDel
      }
      ...
      psa6_subseq    <- as.data.frame(tapply(psa5$psa4, psa5$CIGAR, function(x){paste(x, collapse = ",")})) #the subtype of sequence
      ...
      ### 13. get full sequence ###############################################################
      psa4_all <- unlist(psa4) #for i
      psa2_all <- compareStrings(psa2[[f]]) #for i
      
      names(psa2_all) <- psa4_all
      psa6_full_sequence <- psa2_all[psa6_maxsubseq]    #get full sequence from alignmented Strings
      
      ### 14. sumarizing data ################################################################
      CIGER_count_full <- cbind(psa6_maxsubseq, psa6_num, psa6_par, psa6_CIGAR, psa6_deletions, psa6_insertions, psa6_shifts, psa6_full_sequence)
      colnames(CIGER_count_full) <- c("Target window sequence", "# of reads", "% of reads", "CIGAR", "Deletions","Insertions", "In-/Out- of frame", "Full amplicon sequence")
      CIGER_count_full$`Full amplicon sequence` <- gsub("\\?", "N", CIGER_count_full$`Full amplicon sequence`)
    ...
    return(list(CIGER_count_filtered=CIGER_count_filtereds, 
